...
class ChatRepository {
  ...
  Future<void> markMessageAsSeen({
    ...
    try {
      ...
      await _db
          .chatMessages(userId: userId, chatId: receiverId)
          .doc(messageId)
          .update({
        'isRead': true,
      });
      await _db
          .chatMessages(userId: receiverId, chatId: userId)
          .doc(messageId)
          .update({
        'isRead': true,
      });
  ...
  Future<void> saveMessage({
    ...
    final newMsg = MessageModel(
      ...
      repliedMessageType:
          reply.isNone() ? MessageType.text : reply.unwrap().type,
    );
    if (isGroup) {
      await _db.groups
          .doc(receiverId)
      ...
      await _db
          .chatMessages(userId: sender.uid, chatId: receiverId)
          .doc(id)
          .set(newMsg);
      await _db
  ...
  Future<void> sendFileMessage({
    ...
    try {
      ...
      String url = await _storage.uploadFile(
        path:
            'chat/${type.type}/${sender.uid}/$receiverId/${time.millisecondsSinceEpoch}',
        file: file,
      );
  ...
  Future<void> sendTextMessage({
    ...
    log("sendTextMessage :>>: $message $reply $sender");
    try {
